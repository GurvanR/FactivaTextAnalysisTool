...
def convert_date_factiva(df: pd.DataFrame, column: str = "PD",  language: str = "english") -> None :
    ...
    # Remove rows with 'Missing Data' in the 'PD' column
    df = df[df[column] != 'Missing Data']

    df[column] = df[column].apply(lambda x: datetime.strptime(x.strip(), '%d %B %Y'))
    df[column] = pd.to_datetime(df[column]) # Seems that the property of being a datetime value vanishes at the csv conversion.
...
def FactivaRTFtoDF( rtf_file_path: str, 
    ...
    if date_time_format : 
        df = convert_date_factiva(df, language=datetime_language)
        ...
        for file_name in os.listdir(rtf_folder_path):
            ...
            if os.path.isfile(file_path):
                ...
                if os.path.splitext(file_path)[1].lower() == '.rtf':
                    dataframes.append(FactivaRTFtoDF(rtf_folder_path + '/' + file_name, output_format='pd.DataFrame', 
                                                    output_file_name= output_file_name, desired_acronyms= desired_acronyms,
                                                    desired_fields = desired_fields, join_LP_TD = join_LP_TD,
                                                    date_time_format=date_time_format, datetime_language= datetime_language))
...
def FactivaFolderToCSV():
    def select_folder():
        folder_path = filedialog.askdirectory()
        if folder_path:
            output_file = os.path.join(folder_path, file_name_entry.get() or 'FactivaPostProcessed')
